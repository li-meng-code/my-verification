...
function bit svt_err_catcher::report(bit silent = 0);
  ...
  foreach (messages_to_demote_removed[msg_indx]) begin
    if (demoted_messages_limit[messages_to_demote_removed[msg_indx]] == 0) begin
      if (demoted_messages_count[messages_to_demote_removed[msg_indx]] == 0) begin
        if (!silent) begin
          `svt_xvm_note("report", $sformatf("Message text `%s` was removed, and was not detected", messages_to_demote_removed[msg_indx]));
        end
        if (messages_to_demote_removed_limit_check.exists(messages_to_demote_removed[msg_indx])) begin
          catcher_status = 0;
        end
      end
      else begin
        if (!silent) begin
          `svt_xvm_note("report", $sformatf("Message text `%s` was removed, and was detected %0d times", messages_to_demote_removed[msg_indx], demoted_messages_count[messages_to_demote_removed[msg_indx]]));
        end
      end
    end
    else begin
      if (!silent) begin
        `svt_xvm_note("report", $sformatf("Message text `%s` was removed, and was detected %0d times (expected %0d)", messages_to_demote_removed[msg_indx], demoted_messages_count[messages_to_demote_removed[msg_indx]], demoted_messages_limit[messages_to_demote_removed[msg_indx]]));
      end
      if (messages_to_demote_removed_limit_check.exists(messages_to_demote_removed[msg_indx]) && demoted_messages_limit[messages_to_demote[msg_indx]] != demoted_messages_count[messages_to_demote[msg_indx]]) begin
        catcher_status = 0;
      end
    end
  end

  foreach (ids_to_demote_removed[id_indx]) begin
    if (demoted_id_limit[ids_to_demote_removed[id_indx]] == 0) begin
      if (demoted_id_count[ids_to_demote_removed[id_indx]] == 0) begin
        if (!silent) begin
          `svt_xvm_note("report", $sformatf("Message ID `%s` was removed, and was not detected", ids_to_demote_removed[id_indx]));
        end
        if (ids_to_demote_removed_limit_check.exists(ids_to_demote_removed[id_indx])) begin
          catcher_status = 0;
        end
      end
      else begin
        if (!silent) begin
          `svt_xvm_note("report", $sformatf("Message ID `%s` was removed, and was detected %0d times", ids_to_demote_removed[id_indx], demoted_id_count[ids_to_demote_removed[id_indx]]));
        end
      end
    end
    else begin
      if (!silent) begin
        `svt_xvm_note("report", $sformatf("Message ID `%s` was removed, and was detected %0d times (expected %0d)", ids_to_demote_removed[id_indx], demoted_id_count[ids_to_demote_removed[id_indx]], demoted_id_limit[ids_to_demote_removed[id_indx]]));
      end
      if (ids_to_demote_removed_limit_check.exists(ids_to_demote_removed[id_indx]) && demoted_messages_limit[messages_to_demote[id_indx]] != demoted_messages_count[messages_to_demote[id_indx]]) begin
        catcher_status = 0;
      end
    end
  end
...
VqUaJOVhjGHxgHiCFH5v+qXEu2eUIi2rLfSwn4ynsFheh3R32dmWrwt7tHc6ZMgq
ktmUH06MuzAisHX4xf9vD0xWQiS582BOqM6A9gnogACT4Ite3Y/GtOFoqVEUSj3r
iGF7t7kMxbobhiS7UMIbFjUuzGv7AA8ycUqrOxDCbS4=
`pragma protect data_block encoding = ( enctype = "base64", bytes = 83        )
tFvDJkIvvQKcZ4byJ1jiy5tdbEN1Vveun3H7iP5S0cNJ178vQJF9sKrwqINI73Qc
MXBCe9yJ+d2QJ+TP61APISrVPRIZ05pD6cn7IhNoDsVmiGWzN31Ki9TfxCwuCdKw
