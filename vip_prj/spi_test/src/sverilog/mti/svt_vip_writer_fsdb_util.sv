...
function void svt_vip_writer_fsdb_util::fsdb_set_object_field_value_time( longint unsigned transaction_id,
  ...
  begin
    ...
    // Only write out the expected value if it is present and it is different from the actual value.
    if ( has_expected && field_value != expected_field_value ) begin
    //Added simulator specific check as in verdi STAR 9001385966, the new option "+time_unit" is added for VCS DPI and PLI transaction task only.
    //So it’s invalid option in other simulators.”
`ifdef VERDI_DEF_TRANS_DPI_ADD_REAL_ATTR_WITH_EXPECTED_VALUE
...
bWiIwfN4JYkIfJwhU0tWxYCvIFpWQ7izDJckS2S6FUlCOqsQbi9O706hEnYpKgKy
PIqFhUQEk3c0VdZn+t1XdXk8ajJ/A8hmKOFyPdOtDntkqnmo6bG7o+2OJ3EBCE8a
rjS5D7B0P61PLjB1qWz7d3xa+R9Ol/wbqNzv8F5zDfc=
`pragma protect data_block encoding = ( enctype = "base64", bytes = 83        )
9l9fKPAtMJRszDRZJo38ZV+/yMq7EAb1M6E63v9QX2EFwBrOlsiDvyVyyLrq/jeV
oJk4+qW9RtPl4lf72TZ8IkfTSy1PKqmIEkqeOfhNg+Mm2641RZtLR2WNbTESiACx
